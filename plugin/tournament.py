"""
Hidden Tournaments (private brackets)
=====================================
...
Design (agreed):
  - Foundation: native Brackets. Teams/Users already have `bracket_id`.
  - Exclude from general scoreboard: members of a hidden bracket get `hidden=True`.
    Core `get_standings()` filters `hidden==False` for non-admins, so the whole
    public surface (board, graph, top-N) drops them automatically — no core patch.
  - Private dashboard: /tournament renders standings for the caller's own hidden
    bracket via get_standings(bracket_id=…, admin=True) (admin=True re-includes the
    hidden members). Access = a member of that bracket, or a site admin.
  - Assignment: admin does it by hand at /admin/tournaments (sets bracket_id + hidden).
...
def hidden_bracket_ids():
    ...
    try:
        ...
        logger.warning("tournament: bad %s config: %r", CONFIG_KEY, raw)
        return set()
...
# bracket — instead of a global menu item every one of 500 players would see.
...
def _tournament_ctx():
    try:
        ...
        logger.debug("tournament: ctx processor failed", exc_info=True)
    return {}
...
def _standings_rows(bracket_id):
    ...
    return [
        {"place": i + 1, "account_id": s.account_id, "name": s.name, "score": s.score}
...
@tournament_bp.route("/admin/tournaments")
@admins_only
...
@tournament_bp.route("/admin/tournaments/<int:bracket_id>/toggle", methods=["POST"])
@admins_only
...
@tournament_bp.route("/admin/tournaments/<int:bracket_id>/add", methods=["POST"])
@admins_only
...
# ── templates (extend the active theme's base) ───────────────────────────────────
...
def init(app):
    """Wire the hidden-tournament feature into the CTFd app (called from load())."""
    app.register_blueprint(tournament_bp)
    app.context_processor(_tournament_ctx)
    logger.info("Hidden Tournaments feature loaded")
